...
async def createCredit(ctx, member): #Adds a user to the credit system
    log.com(ctx)
    try:
        with open(c.creditPath, 'r') as f: users = json.load(f) #Opens the JSON file Credit and assigns it to f / #Loads our data and assigns it to users
    ...
    with open(c.creditPath, 'w') as f: #Opens the JSON file Credit and assigns it to f
...
async def credit(ctx, credit, member, reason):
    ...
    try: users[f'{member.id}']['credit'] += credit # writes credit amount to the user and selected user ID 
        ...
        with open(c.creditPath, 'r') as f: users = json.load(f) # Reloads credit.json
    ...
    with open(c.creditPath, 'w') as f: json.dump(users, f) # If all is successful the credit is written to the .json file 
...
async def purge(ctx, amount):
    ...
    if amount <= c.maxPurge+1 and amount > 0: await ctx.channel.purge(limit=amount); log.me(f"{amount-1} message(s) were cleared by the user")
...
async def mute(ctx, member):
    log.com(ctx)
    try: role = discord.utils.get(member.guild.roles, name=c.muteRole); await member.add_roles(role) # Attempts to get the servers muted role
...
async def unmute(ctx, member):
    ...
    try: role = discord.utils.get(member.guild.roles, name=c.muteRole); await member.remove_roles(role) # Attempers to get the servers muted role 
    ...
    try: await member.create_dm(); await member.dm_channel.send(f"Hi {member.name} {c.unmuteMessage}") # Attempts to DM a user
    except: await ctx.send("User's DM couldn't be reached") # If it cant then it lets the end user know that they cant be reached
...
async def ban(ctx, member, reason):
    ...
    try: await member.create_dm(); await member.dm_channel.send(f"Hi {member.name} {c.banMessage}")
    except: await ctx.send("User's DM couldn't be reached") # If it cant then it lets the end user know that they cant be reached
